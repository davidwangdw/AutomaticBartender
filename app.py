...
def make_drink(recipe):
    ...
    while time_elapsed <= longest_time_needed:
        relays_to_activate = [liquid_sources_dict[ingredient] for ingredient, time_left in recipe.items() if time_left > 0]
        relays_to_deactivate = [liquid_sources_dict[ingredient] for ingredient, time_left in recipe.items() if time_left == 0]
